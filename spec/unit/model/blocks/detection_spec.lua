...
describe('detection', function()
  ...
  describe('detect_type', function()
    ...
    describe('quote detection', function()
      ...
      it('should NOT detect > as quote (> is for toggle)', function()
        local block_type, prefix = detection.detect_type('> text')
        -- > is toggle, not quote
        assert.are.equal('toggle', block_type)
        assert.are.equal('> ', prefix)
      end)
    end)

    describe('toggle detection', function()
      it('should detect "> " prefix as toggle', function()
        local block_type, prefix = detection.detect_type('> toggle text')
        assert.are.equal('toggle', block_type)
        assert.are.equal('> ', prefix)
      end)

      it('should require space after >', function()
        local block_type, prefix = detection.detect_type('>no space')
        assert.is_nil(block_type)
        assert.is_nil(prefix)
      end)

      it('should detect empty toggle', function()
        local block_type, prefix = detection.detect_type('> ')
        assert.are.equal('toggle', block_type)
        assert.are.equal('> ', prefix)
      end)
  ...
  describe('get_prefix_for_type', function()
    ...
    it('should return "> " for toggle', function()
      local prefix = detection.get_prefix_for_type('toggle')
      assert.are.equal('> ', prefix)
    end)
